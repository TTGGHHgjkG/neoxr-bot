         ...
         if (style === 1) {
            ...
            if (text) {
               ...
               keys.sort((a, b) => a.localeCompare(b)).map((v, i) => sections.push({
                  ...(/download|fun|util/.test(v) ? label : {}),
                  rows: [{
                     title: Func.ucword(v),
                     description: `Ada ${Func.arrayJoin(Object.entries(plugins).filter(([_, x]) => x.run.usage && x.run.category == v.trim().toLowerCase() && !setting.hidden.includes(x.run.category.toLowerCase())).map(([_, x]) => x.run.usage)).length} perintah yang tersedia`,
               ...
               client.sendIAMessage(m.chat, buttons, m, {
                  header: Func.greeting() + ` ${Func.censorName(m.pushName)} 👋`,
                  content: print
               })
            }
         }
